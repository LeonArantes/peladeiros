...
const PlayerDetail = () => {
  ...
  const fetchPlayer = async () => {
    try {
      setLoading(true);
      const playerData = await userService.findById(id);
      playerStatsService.debugPlayerStats(id);
      if (playerData) {
        ...
        toast({
          ...
          description: "O jogador solicitado não foi encontrado.",
          status: "error",
          duration: 5000,
          isClosable: true,
        });
  ...
  const onSubmit = async (data) => {
    if (!isAdmin()) {
      toast({
        title: "Acesso negado",
        description: "Apenas administradores podem editar jogadores.",
        status: "error",
        duration: 3000,
        isClosable: true,
      });
                    ...
                    <VStack spacing={3}>
                      ...
                      <Text fontSize="xs" color="gray.400" textAlign="center">
                        {stats.summary.matchesPlayed > 0
                          ? "Participou de partidas, mas dados de vitórias/derrotas não estão disponíveis."
                          : "Ainda não participou de divisões de times."}
                      </Text>
                    </VStack>
                  </Center>
                ) : (
